...
def get_dashboard_insights(current_user):
    try:
        ...
        latest_summary = Summary.query.filter_by(user_id=current_user.id).order_by(Summary.year.desc(), Summary.month.desc()).first()
        portfolio = Portfolio.query.filter_by(user_id=current_user.id).order_by(Portfolio.created_at.desc()).first()
...
@token_required
def get_learning_plan(current_user):
    try:
